...
from pose_estimation.models.train import train_model as pose_train_model
from pose_estimation.inference.file import infer_folder as pose_infer_folder
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '-t', '--task',
        choices=[
            'pretrain', 'hpo', 'train', 'eval', 'viz',
            'inference', 'webcam', 'all'
        ],
...
def main() -> None:
    ...
    # ======================
    # Webcam pipeline (Gaze + Pose)
    # ======================
    if args.task == 'webcam':
        from app.webcam.main import main as webcam_main
        LOGGER.info('▶️ [GAZE & POSE WEBCAM]')

        webcam_main(
            model_path      = infer_cfg['weights'],
            pose_model_path = infer_cfg['pose_weights'],
    ...
    if args.domain == 'gaze':
        ...
        if args.task == 'pretrain':
            ...
            ds_root, yaml_p = gaze_download_dataset(
                pre_cfg,
                dest_dir=datasets_dir / 'pretrain'
            )
            params = {
                'data': pre_cfg,
                'model': {
        ...
        if args.task == 'train':
            ...
            ds_root, yaml_p = gaze_download_dataset(
                data_cfg,
                dest_dir=datasets_dir / data_cfg['project_name']
            )
            ...
            hpo_file: Path = output_root / 'hpo' / 'hpo_best_params.yaml'
    ...
    if args.domain == 'pose':
        # TRAIN
        if args.task == 'train':
            LOGGER.info('▶️ [POSE TRAIN]')
            pose_train_model(
                common_cfg,
                pose_cfg,
                Path(pose_cfg['dataset_yaml'])
            )
            return

        # INFERENCE / ALL
        if args.task in ('inference', 'all'):
            ds_root    = Path(pose_cfg['dataset_yaml']).parent
            img_folder = ds_root / 'test' / 'images'
            inf_out    = output_root / 'pose' / 'inference'
            inf_out.mkdir(parents=True, exist_ok=True)
            LOGGER.info('▶️ [POSE INFERENCE]')
            pose_infer_folder(
                model_path   = pose_cfg['weights'],
